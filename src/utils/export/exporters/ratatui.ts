  ...
  if (node.type === 'Screen' || node.type === 'Box' || node.type === 'Grid' || node.type === 'Modal') {
    ...
    if (node.layout.type === 'absolute') {
      for (const child of node.children) {
        ...
        out += generateRatatuiNode(child, indent + 1, childArea);
      }
      return out;
    }
    ...
    if (node.type === 'Grid') {
      ...
      for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < columns; col++) {
          ...
          // F8 — pass indent + 1
          if (child) out += generateRatatuiNode(child, indent + 1, `${colChunks}[${col}]`);
        }
      }
      return out;
    }
    ...
    node.children.forEach((child, i) => {
      // F8 — pass indent + 1
      out += generateRatatuiNode(child, indent + 1, `${chunks}[${i}]`);
    });
    return out;
  }
...
// F2 — complete Rust string escaping including \r and \t
